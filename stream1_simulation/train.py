...
def main():
    ...
    checkpoint_dir = os.path.join(STREAM1_DIR, "checkpoints", f"run{args.run_id}")
    os.makedirs(checkpoint_dir, exist_ok=True)

    run = wandb.init(
        project="rl-policy-debugger",
        name=f"run{args.run_id}",
        group="stream1-training",
        mode=args.wandb_mode,
        sync_tensorboard=True,
        config={"run_id": args.run_id, "seed": args.seed, "timesteps": args.timesteps},
    )
    ...
    model = PPO(
        ...
        seed=args.seed,
        verbose=1,
        tensorboard_log=os.path.join(STREAM1_DIR, "tb_logs", f"run{args.run_id}"),
    )
